...
| **Se** | *The Naked Eye* — a wide-dilated iris-aperture holding a live field of drifting, glinting particulate; through the opening, that same field runs sharper, faster, and more saturated than it does outside the ring — the same world, *more so*. Nothing mediates it and nothing is kept: what leaves the frame is gone within a breath | Instant, saccadic, appetitive; locks onto any motion within a single frame, grows crisper as the field grows wilder, and turns restless — dilating, hunting — when the field runs flat. The cursor is the most vivid object in its world: tracked with zero lag, greeted with a leap of amber tracers |
| **Si** | *The Still Pool* — a sealed pool seen from above, its depth banded into concentric sediment strata: every past inflow laid down as a ring, the oldest fused bright at the core. Each admitted drop sinks to the stratum it matches and lights the whole ring on contact; rings it feeds thicken, rings left unfed slowly dim — but never quite vanish | Measured, layered, ceremonial; a soft chime and a circumference-wide glow when past and present match, a flat dissonant shiver across the surface when the familiar arrives changed |
...
| **Fe** | *The Resonance Field* — a ring of independent oscillators *outside* the chamber, coupled to each other by a lattice Fe draws between them; the glyph reads the field's phase and conducts it toward synchrony. Its nucleus has no colour of its own: hue and brightness are the amplitude-weighted mean of whoever is currently in the ring | Warm, responsive, other-directed; concord sharpens the interference fringes into one sweeping wavefront, disharmony makes them beat, and an unresolvable split drives a standing null straight through the centre. Alone, the nucleus desaturates to grey — the mean of an empty set |
...
**The amber mirrors (Se/Si).** Se and Si are two fates of the same moment, and their glyphs are built to say so. *The Naked Eye* is all opening — the extraverted open ring of §1.3 taken to its limit, a rim that is nearly all gap — and its one trick is heightening: the world seen *through* Se is visibly more vivid than the same world beside it, which is the function's entire argument made without a caption. (It renders as a warm amber diaphragm, iris-like but never anatomical.) *The Still Pool* is all keeping — the introverted double ring multiplied into dozens of strata, intake metered to a drip through the sealed rim, and the luminous introverted core reinterpreted: the deepest, oldest layers sit fused and bright at the center, first impressions as the load-bearing middle of the self. The seam between them is deliberate: the afterimages the Eye discards are exactly the material the Pool is made of. One streams the moment; the other banks it. Set side by side in Zone G they read as an eye and a pool — water that looks, and water that keeps.

**The rose mirrors (Fe/Fi).** The feeling pair is built on a single inversion, and the two glyphs exist to state it: **Fi's perimeter is computed from its centre; Fe's centre is computed from its perimeter.** *The Tuning Fork* holds one fixed core tone, sealed behind a closed boundary, and every arrival is struck against it — the colour is intrinsic, and an experience is admitted only if it rings true. *The Resonance Field* holds no tone at all. Its working objects — the carrier nodes standing for other people — sit *outside* its own rim, the coupling lattice it builds is drawn between them rather than inside itself (the sharpest available statement of what extraverted judging means), and its nucleus takes the circular mean of whatever the ring is doing. This is why the two functions fail in opposite directions, and why the pages teach each other: isolate Fi and the core still burns; isolate Fe and there is no average left to take, so the glyph goes grey on screen. Their labs cross-listen for the same reason the amber pair does — the celebration that peaks Fe's meters barely moves Fi's, the covert hostility Fe can feel but not name registers on Fi's instruments instantly and unambiguously, and the solitude that severs Fe is where Fi rests.
...
Generalization note: each judging function gets its own verification variant (Te verifies against external results, Fe against the coherence of the field around it, Fi against the core tone); perceiving-function pages replace this zone with a **Perception Fidelity Lab** — specified for Se and Si below, with Ne and Ni to receive variants on the same pattern. The variants differ in *texture*, not just content — Ti's chamber is crystalline (structures fold in or bounce off a lattice), while Fi's is a **mist nebula with fluid dynamics**: its lab spawns a 2×2 of experiences (authentic/inauthentic × good/bad) where authentic experience — joyful or painful — diffuses into the mist, twirls it, and temporarily colours the whole nebula (the tint tracks the colour distribution of what it currently holds, clamped near the core rose); inauthentic-pleasant experience glides along the boundary unabsorbed and is expelled; and inauthentic-violating experience pierces straight through with a wake, a trailing vacuum, and mist bleeding out through the exit wound.

**The Resonance Lab (Fe).** Fe's variant keeps the skeleton — a live chamber, spawnable events, two meters — but what is on trial is neither a claim's coherence nor an experience's authenticity; it is **the state of the field between people**, which is the only place Fe's judgment happens. The chamber runs a ring of coupled oscillators (a Kuramoto model, so behaviour is computed rather than choreographed) and the master metric is the order parameter *R* — one scalar driving halo, fringe sharpness, nucleus luminance, and the pleasure meter at once. Six spawnable scenarios split into harmony and dissonance: an **unspoken sync** that costs nothing and that nobody in the room can see happening; a **collective celebration** that reaches perfect constructive interference and, in the same frame, shows the differentiation readout collapsing — Fe's characteristic failure visible at the moment of its greatest success; a **reconciliation** of a split field, gated so it can only be spawned as a *response*, costing four times any other event and paying out with the site's best single animation. Against them: **covert hostility**, where two nodes mask their phase so the concord readout stays high while stress climbs anyway — the meters disagreeing, which is the entire content of the scenario and the most characteristic Fe experience there is; an **unresolvable deadlock**, two clusters locked antiphase with a standing null through the centre and the effort meter pegged at zero benefit; and **forced isolation**, which severs the ring and lets the nucleus desaturate to grey, then — the second beat — has Fe begin conducting to phantom nodes it invented, paying full cost for no coupling, which is rumination drawn exactly as it works. Fe's meters are *System Stress* and **Resonance Pleasure**, and a **trust-gap** readout (believed concord minus actual concord) is the instrument that makes covert dissonance legible; the same read-noise machinery runs ambiently in Zone B, where the two needles sit on top of each other at Dominant and have visibly separated by Trickster.

**The Perception Fidelity Lab (Se and Si).** For perceiving functions the lab keeps the same skeleton — a live chamber, three spawnable stimuli, two meters — but what is on trial is not a claim's coherence; it is the *quality of contact between psyche and world*. Stress keeps its meter; the pleasure meter renames per function (**Contact Pleasure** for Se, **Recognition Pleasure** for Si). The two amber labs are deliberately built as inverse instruments, and each carries a small **cross-listen inset** — a ghosted copy of the sibling's meters reacting to the same event — so the deepest lesson (one element, two attitudes, opposite economies) is taught by two needles moving in opposite directions.

**Se — the Contact Lab.** The chamber runs against a live stimulus field whose master control is a user-owned **intensity slider**, from *empty room* to *street carnival*. The slider alone teaches Se's signature economy before anything is spawned: as intensity rises, Contact Pleasure climbs and rendering fidelity *rises with it* — stimulation produces focus, not overload — while dragging toward empty sends Stress climbing as the aperture dilates and hunts. For Se, nothing happening is the emergency. Three spawnable events:

- **A flicker in the crowd:** one element in a dense field shifts for a few hundred milliseconds. The aperture saccades to it and tags it within a single frame; pleasure ticks up, stress stays flat. A ghosted side-pane replays the identical field through a non-Se blur, where the flicker passes unseen — the effect of having Se, shown by subtraction. Caption: "change registers before thought — the fastest event in the psyche."
- **A closing window:** a gap opens in the field and begins shutting. The event honors the slot currently selected in Zone B: at Dominant, Se's response particle threads the gap with room to spare; re-spawned with Se dragged to Inferior, the ~700 ms latency of §3.1 fires the response at a window that has already shut. Position → missed openings, as one picture. Caption: "Se doesn't just see the present — it arrives in time to use it."
- **Lights out:** the field empties. Within a second the chamber holds nothing but a fading afterimage — tagged with a quiet glint labeled for Si, *the stratum its sibling would have kept* — and Stress begins a steady climb the longer the emptiness holds. Caption: "the price of full contact: Se keeps nothing, and it cannot stand nothing."

**Si — the Recognition Lab.** The pool runs with its strata visible; every spawn is a drop of the present, interrogated by the depths. Three arrivals:

- **The familiar, on time:** a drop matching a bright ring descends in a beeline; the ring lights its full circumference with a soft chime, the stratum thickens by a hair, and Recognition Pleasure hums. Re-spawning it is the comfort ritual: a per-encounter cost readout falls with each pass while pleasure holds — rituals shown as load-bearing, not lazy. Caption: "recognition is instant and nearly free — Si runs the body's checklists at a glance."
- **The familiar, changed:** a drop that matches a stratum at ninety percent reaches its ring and *refuses to merge* — strobing between stored color and observed color, sounding a flat beat-frequency, rippling the whole surface. Stress spikes, and a **diff readout** names exactly what is off ("pitch a third low; edge two shades cool"): Si's alarm is specific, never vague. The user resolves it — **accept**, and a thin sub-ring forms beside the old stratum, dim until re-fed over later encounters (change is possible, just expensive); or **reject**, and the drop is expelled back through the rim while the old ring re-asserts a shade brighter. Distress → comparison → verdict → slow re-encoding, as one continuous picture. Caption: "Si is the psyche's proofreader — first to know when the world deviates from the record."
- **The unprecedented:** a drop matching nothing circles without finding a depth; the pool dims and the rings contract a shade — the flinch — while Stress climbs slowly (novelty is costly, not catastrophic). It finally settles at the rim as a thin, dim, provisional band. Spawning it again and again brightens the band, shortens the flinch, and drops the cost readout: familiarity being manufactured in front of the user. Caption: "nothing is comfortable the first time; almost anything can become comfortable."
