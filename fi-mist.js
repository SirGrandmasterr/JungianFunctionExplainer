/* ============================================================
   ...
   field the CPU proxy mist uses: radial home springs, slow
   swirl, curl-ish turbulence, vortices from absorbed
   experience, bullet wakes with trailing vacuum, radial pushes
   from unabsorbed pellets, and colour splats that the nebula
   slowly digests back toward its core rose.
...
uniform int  uVortN;
uniform vec4 uBul[2];      // xy = pos, zw = unit dir
uniform float uBulOn[2];   // wake scale, 0 = off
uniform vec4 uPush[2];     // xy = pos, z = radius, w = strength
uniform int  uPushN;
uniform vec4 uSplat[4];    // xy = pos, z = radius, w = strength
...
void main(){
  ...
  // radial spring toward this grain's home radius (keeps the inward-thickening profile)
  float target = aHomeR * uR;
  v += rad * (target - d) * uDt * 2.6;

  // slow bulk rotation, faster at the rim
  v += tng * uSwirl * (0.35 + 0.65 * d / max(uR, 1.0)) * uDt;

  // gentle curl-ish turbulence so the mist is alive even at rest
  v += vec2(
  ...
  // vortices — twirl from absorbed experience
  for (int i = 0; i < 8; i++){
    if (i >= uVortN) break;
  ...
  // bullets — wake shove + trailing vacuum
  for (int i = 0; i < 2; i++){
  ...
  // radial pushes — the mist parting around what it refuses to mix with
  for (int i = 0; i < 2; i++){
    if (i >= uPushN) break;
  ...
  // viscous damping, integrate
  v *= exp(-3.1 * uDt);
  p += v * uDt;

  // colour: digest foreign colour back toward the core rose…
  c = mix(c, uBase, clamp(uDt * 0.055, 0.0, 1.0));
  // …and take on the colour of what has just been absorbed nearby
  for (int i = 0; i < 4; i++){
    if (i >= uSplatN) break;
...
const STRIDE = 9 * 4;   // px py vx vy homeR seed r g b
...
class MistGPU {
  constructor(canvas, opts = {}){
    ...
    try {
      ...
      this.uni = {};
      for (const n of ['uDt','uTime','uR','uSwirl','uNoise','uAwake','uBase','uVort','uVortN',
                       'uBul','uBulOn','uPush','uPushN','uSplat','uSplatCol','uSplatN'])
  ...
  _initBuffers(R0){
    ...
    for (let i = 0; i < N; i++){
      const u = rnd();
      const homeR = 0.08 + 1.06 * Math.pow(u, 1.5);   // thick toward the centre, sparse halo outside
      const a = rnd() * Math.PI * 2;
      const o = i * 9;
      data[o]   = Math.cos(a) * homeR * R0;
      data[o+1] = Math.sin(a) * homeR * R0;
  ...
  /* state: {R, swirl, mood, noise, awake, shift, alpha, base:[r,g,b] 0..1, center:{x,y},
            vortices:[{x,y,s,life}], bullets:[{x,y,dx,dy,on}], pushes:[{x,y,r,s}],
            splats:[{x,y,r,s,col:[r,g,b]}]} */
  frame(dt, st){
    if (!this.ok) return;
    const gl = this.gl;
    ...
    for (let i = 0; i < vn; i++){
      ...
      vorts[i*4] = v.x; vorts[i*4+1] = v.y; vorts[i*4+2] = v.s; vorts[i*4+3] = v.life;
    }
    gl.uniform4fv(this.uni.uVort, vorts);
    gl.uniform1i(this.uni.uVortN, vn);
    const buls = new Float32Array(8), bon = new Float32Array(2);
    for (let i = 0; i < Math.min(st.bullets.length, 2); i++){
      const b = st.bullets[i];
      buls[i*4] = b.x; buls[i*4+1] = b.y; buls[i*4+2] = b.dx; buls[i*4+3] = b.dy;
      bon[i] = b.on;
    }
    gl.uniform4fv(this.uni.uBul, buls);
    gl.uniform1fv(this.uni.uBulOn, bon);
    const pushes = new Float32Array(8);
    const pn = Math.min(st.pushes.length, 2);
    for (let i = 0; i < pn; i++){
      const p = st.pushes[i];
      pushes[i*4] = p.x; pushes[i*4+1] = p.y; pushes[i*4+2] = p.r; pushes[i*4+3] = p.s;
    }
    gl.uniform4fv(this.uni.uPush, pushes);
    gl.uniform1i(this.uni.uPushN, pn);
    const spl = new Float32Array(16), splc = new Float32Array(12);
    const sn = Math.min(st.splats.length, 4);
    for (let i = 0; i < sn; i++){
      const s = st.splats[i];
      spl[i*4] = s.x; spl[i*4+1] = s.y; spl[i*4+2] = s.r; spl[i*4+3] = s.s;
      splc[i*3] = s.col[0]; splc[i*3+1] = s.col[1]; splc[i*3+2] = s.col[2];
    }
    gl.uniform4fv(this.uni.uSplat, spl);
    gl.uniform3fv(this.uni.uSplatCol, splc);
    gl.uniform1i(this.uni.uSplatN, sn);
